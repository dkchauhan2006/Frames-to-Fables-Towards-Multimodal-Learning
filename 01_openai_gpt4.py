"""
Story Generation using OpenAI GPT-4o & GPT-4o-mini
...
def generate_story(model_name: str, client: OpenAI) -> str:
    """Generate a story using the specified OpenAI model."""
    print(f"\n{'='*60}")
    ...
    response = client.chat.completions.create(
        model=model_name,
        messages=[
            {"role": "system", "content": "You are a creative and empathetic children's story writer."},
    ...
    print(f"✓ Story generated | Tokens used: {tokens_used}")
    return story
...
def main():
    ...
    if not api_key:
        raise ValueError("OPENAI_API_KEY not set. Export it before running.")
    ...
    for model_id, model_label in models.items():
        try:
            story = generate_story(model_id, client)
    ...
    # Save combined JSON
    with open("stories/openai_stories.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\n✓ All stories saved to stories/openai_stories.json")
    return results
